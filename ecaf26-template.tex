...
\author{Hortence Phalonne Yiepnou Nana}
...
DP~\cite{caton2024fairness}  ensures that subpopulations (for example based on gender) are equitably represented in the selected set, even if the groups differ in performance. So, the selection probabilities for different groups will be proportional to their representation in the population. For a policy \(\pol\), it is defined as:
\begin{equation}
...
Specifically, through multi-stage experiments, we looked at cases where policy updates are delayed, and found that even with less frequent updates, neural network still performs well  but not logistic policy. Finally, comparing against simple baselines like GPA or fixed models confirmed the value of letting the system learn and adjust. Among all approaches tested, the neural network-based policy consistently outperformed others, including the logistic one, showing a stronger ability to adapt and improve over time. 
